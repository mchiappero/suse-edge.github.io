...
* With the included RKE2 CAPI provider version, it is now possible to define dual-stack CAPI clusters. Previously only single stack deployments were possible due to a https://github.com/rancher/cluster-api-provider-rke2/pull/452[bug that filtered additional CIDRs] for both Pods and Services. Please refer to https://github.com/suse-edge/atip[https://github.com/suse-edge/atip] for examples on how to configure IPv4, IPv6 and dual-stack downstream clusters (currently limited to single host deployments as a tehnical preview).

== Known Limitations

* A bug affects clusters using static IPs on a network where DHCP servers and/or RAs are also present: any static network configuration will only apply to the provisioned host and will not be in effect during the host discovery and enrollment phase. Please refer to the https://github.com/suse-edge/atip/tree/main/telco-examples/edge-clusters/dhcp-less/dual-stack/single-node#readme[ATIP repository] for more details and updates.
